...
const steps = [
  { n: "1", t: "Send your request", d: "Fill the form — it reaches Imene directly on WhatsApp." },
  { n: "2", t: "Confirm slot & payment", d: "You'll agree on the time and payment method (CIB/EDAHABIA & international options coming online soon)." },
  { n: "3", t: "Meet on video call", d: "A focused session in Arabic, French or English — with clear next steps to apply right away." },
];
        ...
        <div className="mt-6 max-w-2xl">
          ...
          <div className="mt-5 flex flex-wrap gap-2 text-[11px] font-medium tracking-wide text-ink-soft uppercase">
            {["Paid session", "Video call", "AR · FR · EN"].map((c) => (
              <span key={c} className="rounded-full border border-camel/50 bg-ivory px-2.5 py-1">
                {c}
              </span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {plans.map((p) => (
              ...
              >
                ...
                <p className={`text-xs ${p.highlight ? "text-cream/60" : "text-ink-soft"}`}>
                  {p.length}
                </p>
                ...
                <ul className={`mt-5 space-y-2 text-sm ${p.highlight ? "text-cream/80" : "text-ink-soft"}`}>
                  {p.features.map((ft) => (
